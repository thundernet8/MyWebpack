...
class EntryTaskManager {
    constructor(mpkConfig, compiler, devMiddleware, hotMiddleware, allEntries, builtEntryNames) {
        this.allEntries = [];
        this.allEntryNames = [];
    ...
    hookupCompiler() {
        const { mpkConfig, compiler, emitter, devMiddleware, hotMiddleware, allEntries, builtEntryNames } = this;
        compiler.plugin("make", (compilation, done) => {
            ...
            if (newEntryNames.length > 0) {
                promise = Promise.all(newEntryNames.map(n => {
                    ...
                    return entry_1.addWebpackEntry(compilation, this["context"], e.name, e.path).then(() => e);
                })).then((entries) => {
                    entries.forEach(e => {
        ...
        compiler.plugin("done", (_err, c) => {
            ...
            if (this.htmlPageQueue.length > 0) {
                this.htmlPageQueue.forEach(page => {
                    compiler.apply(base_1.getHtmlWebpackPluginInstance(mpkConfig, "index.html", page));
                });
                this.htmlPageQueue = [];
...
function start(config) {
    const entryStatus = {};
    build_1.default(config, function (compiler, webpackConfig, allEntries, entries) {
        ...
        const taskManager = new EntryTaskManager(config, compiler, devMiddleware, hotMiddleware, allEntries, entries.map(e => e.name));
