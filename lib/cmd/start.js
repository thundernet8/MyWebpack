        ...
        compiler.plugin("make", (compilation, done) => {
            ...
            if (newEntryNames.length > 0) {
                promise = Promise.all(newEntryNames.map(n => {
                    ...
                    return entry_1.addWebpackEntry(compilation, compiler.context, e.name, hmrEntry);
                })).then(() => { });
            }
            else {
    ...
    return build_1.default(config).then(result => {
        ...
        server.use((req, res, next) => {
            ...
            if (urlObj.pathname.endsWith(".html")) {
                ...
                taskManager
                    .execEntryTask(entryName)
                    .then(next)
                    .catch(err => log_1.default.error(err.message || err.toString()));
            }
            else {
                next();
            }
        ...
        devMiddleware.waitUntilValid(() => {
            taskManager
                .checkPrebuildEntries()
                .then(() => {
                log_1.default.success(`\r\n📡  Starting dev server on ${devServerOptions.host}:${devServerOptions.port}
                            \r\n`);
            })
                .catch(e => {
                throw e;
