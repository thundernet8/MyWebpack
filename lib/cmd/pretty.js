...
function pretty(config) {
    ...
    const walk = () => fs_tools_1.walkSync(path.join(scanDir), ".(ts|tsx|js|jsx|json)$", (path, stats) => {
        if (stats.isFile()) {
            if (/\.(ts|tsx|js|jsx)$/i.test(path)) {
                const code = fs_1.readFileSync(path).toString();
                fs_1.writeFileSync(path, prettier.format(code, scriptPrettierConfig));
                console.log(`Pretty: ${path}`);
            }
            else if (/\.json$/i.test(path)) {
                const code = fs_1.readFileSync(path).toString();
                fs_1.writeFileSync(path, prettier.format(code, jsonPrettierConfig));
                console.log(`Pretty: ${path}`);
            }
        }
    });
