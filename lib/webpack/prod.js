...
function getProdConfig(mpkConfig, output) {
    let prodConfig = base_1.default(mpkConfig, output);
    ...
    if (Array.isArray(entry)) {
        ...
        Object.keys(entry).forEach(key => {
            ...
            if (Array.isArray(chunkEntry)) {
                prodConfig.entry[key] = Array.from(new Set(preEntries.concat(chunkEntry)));
            }
            else {
                prodConfig.entry[key] = Array.from(new Set(preEntries.concat(chunkEntry)));
            }
