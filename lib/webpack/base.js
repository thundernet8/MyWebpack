...
function getHtmlWebpackPluginInstance(mpkConfig, entry) {
    ...
    let templatePath = path.resolve(mpkConfig.root, mpkConfig.mpk.template, entry.template);
    if (!path_1.isFileSync(templatePath)) {
        templatePath = path.resolve(mpkConfig.root, mpkConfig.mpk.template, "index.html");
    }
    return new HtmlWebpackPlugin(Object.assign({}, htmlInjects, {
        title: entry.name,
        filename: path.resolve(mpkConfig.mpk.distPath, entry.name + ".html"),
        template: path.resolve(mpkConfig.root, mpkConfig.mpk.template, entry.template),
        inject: !isDev,
        cache: true,
