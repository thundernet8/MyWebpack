...
function getHtmlWebpackPluginInstance(mpkConfig, inputFilename, outputFilename) {
    ...
    return new HtmlWebpackPlugin({
        ...
        template: path.resolve(mpkConfig.root, mpkConfig.mpk.template, inputFilename),
        inject: true,
        venders
    });
}
...
function default_1(mpkConfig, outputs) {
    ...
    const getPlugins = function () {
        ...
        outputs.forEach(output => {
            plugins.push(getHtmlWebpackPluginInstance(mpkConfig, output.template, output.filename));
        });
        if (isDev) {
